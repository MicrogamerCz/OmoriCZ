...
Scene_OmoriFile.prototype.onSelectInputOk = function() {
  ...
  if (index === 0) {
    ...
    if (StorageManager.exists(saveFileid)) {
      // Show Prompt Window
      this.showPromptWindow('Přepsat záznam?');
...
Window_OmoriFileInformation.prototype.refresh = function() {
  ...
  this.contents.drawText('POZICE ' + id + ':', 10 + 30, -5, 100, this.contents.fontSize);
  // If Valid
  if (valid) {
    ...
    // Draw Level
    this.contents.drawText('ÚROVEŇ:', 275 + 55, 30, 100, 24);
    this.contents.drawText(actor.level, 290 + 55, 30, 70, 24, 'right');
    // Draw Total PlayTime
    this.contents.drawText('ODEHRANÝ ČAS:', 118, 55, 200, 24);
    this.contents.drawText(info.playtime, 295 + 55, 55, 100, 24);
    // Draw Location
    this.contents.drawText('MÍSTO:', 118, 80, 200, 24);
