...
Game_Interpreter.prototype.showNameInputWindows = function(name = "", max = 7, wait = true) {
  $gameVariables.setValue(_TDS_.NameInput.params.nameVariableID, "SUNNY");
  return;
...
Scene_Map.prototype.createNameInputWindows = function() {
  ...
  this._nameInputNameWindow.y = 16;
...
// add 11/10 s
//////////////////////////////////
Window_OmoriInputLetters.hiragana = "あいうえおがぎぐげごかきくけこざじずぜぞさしすせそだぢづでどたちつてとばびぶべぼなにぬねのぱぴぷぺぽはひふへほぁぃぅぇぉまみむめもっゃゅょゎやゆよわんらりるれろを";
Window_OmoriInputLetters.katakana = "アイウエオガギグゲゴカキクケコザジズゼゾサシスセソダヂヅデドタチツテトバビブベボナニヌネノパピプペポハヒフヘホァィゥェォマミムメモッャュョヮヤユヨワンラリルレロヲ";
Window_OmoriInputLetters.hiraganaToKatakana = function (str)
{
	let newStr = "";
	for (let i = 0; i < str.length; i++)
	{
		let chr = str.charAt(i);
		if (Window_OmoriInputLetters.hiragana.includes(chr))
		{
			newStr += Window_OmoriInputLetters.katakana[Window_OmoriInputLetters.hiragana.indexOf(chr)];
		}
		else
		{
			newStr += chr;
...
class VirtualKeyboard extends Window_Selectable {
  ...
  initialize() {
    ...
    const hh = Math.floor(Graphics.boxHeight / 1.3)
  ...
  confirmEntry() {
    ...
    if(entry.length > 0) {
	  if (Window_OmoriInputLetters.hiraganaToKatakana(entry) === LanguageManager.getMessageData("XX_BLUE.OMOCAT").text)
	  {
		$gameSystem.unlockAchievement("YOU_THINK_YOU_RE_CLEVER_HUH")
	  }
	  if (new RegExp($gameSystem._badWords.join("|")).test(Window_OmoriInputLetters.hiraganaToKatakana(entry).toLowerCase()))
	  { // YIN - Bad words check
        //console.log("That's totally inappropriate");
        this.playBuzzerSound();
        return;
...
VirtualKeyboard.JP = {
	default: [
		...
		"マ ミ ム メ モ ッ ャ ュ ョ ヮ",
		"ヤ ユ ヨ ワ ン ー ～ ・ ＝ ☆",
		"ラ リ ル レ ロ ヲ \u3000 {bksp} {hiragana} {confirm}"
	],
	shift: [
		"あ い う え お が ぎ ぐ げ ご",
		"か き く け こ ざ じ ず ぜ ぞ",
		...
		"ま み む め も っ ゃ ゅ ょ ゎ",
		"や ゆ よ わ ん ー ～ ・ ＝ ☆",
		"ら り る れ ろ を \u3000 {bksp} {katakana} {confirm}"
	],
}
...
VirtualKeyboard["{hiragana}"] = "かな"
VirtualKeyboard["{katakana}"] = "カナ"
